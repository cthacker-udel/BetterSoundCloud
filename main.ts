...
app.whenReady().then(() => {
  ...
  var trayicon = nativeImage.createFromPath(
    __dirname + "/app/lib/assets/icon.ico"
  );
  ...
  var trayCtxMenu = Menu.buildFromTemplate([
    {
      label: "Play/Pause",
      type: "normal",
      click: () => mainWindow.webContents.send("appReqMediaPlayPause"),
    },
    {
      label: "Skip",
      type: "normal",
      click: () => mainWindow.webContents.send("appReqMediaNextTrack"),
    },
